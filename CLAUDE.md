...
- **`ModConfig`** — hardcoded constants `enabled`, `tier1MaxLevel` (60), `tier1RanksPerPoint` (3), `tier2RanksPerPoint` (2), `maxSkillBonusPoints` (0), `xpMultiplier` (3f — the XP-gain multiplier read by `SkillXpBoostPatch`; `1.0` = off, independent of the talent-point curve), plus `TalentPointsAtLevel(level)` and `GrantsPointAtLevel(level)` — the shared two-tier formula and its grant-level predicate. No runtime config file — the RoslynCSharp sandbox blocks `System.IO`.
