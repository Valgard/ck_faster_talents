...
- **`FasterTalentsMod` (`IMod`)** — bootstrap; logs init. Calls `BurstDisabler.DisableBurstForSystem<AddSkillValueSystem>()` in `Init()`: the talent-curve patch targets are managed (no Burst), but the XP-boost patch (`SkillXpBoostPatch`) targets the Burst-compiled `AddSkillValueSystem`, so Burst is disabled for that one system.
